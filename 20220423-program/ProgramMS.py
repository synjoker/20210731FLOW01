...
# 水平噪声、数值噪声
stream = VideoGear(source=0, stabilize=para['is_anti_shake'], **solution['options']).start() # To open any valid video stream(for e.g device at 0 index)
...
with open("tVariance_" + timeNow + '/Variance.csv','w',newline='')as csv_file:
    ...
    # read current frame from cap
        ...
        flow_blend_enhance = cv.addWeighted(frame_cur[solution['roi_rect'][2]+para['Noffset']:solution['roi_rect'][3]-para['Noffset'],solution['roi_rect'][0]+para['Noffset']:solution['roi_rect'][1]-para['Noffset']], 1 ,bgr_flow_enhanced,  1 , 0)
        frame_blend = cv.addWeighted(frame_cur, 1 ,blank,  1, 0)#frame_cur.copy()
        ...
        cv.imshow('Area of Intrest Blended', cv.resize(frame_blend  , (1400, 720)))
