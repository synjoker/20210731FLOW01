...
class MyMainWindow(QMainWindow, Ui_MainWindow):
    ...
    def slot_init(self):
        ...
        self.timer_camera_2.timeout.connect(self.show_camera)  # 若定时器结束，则调用show_camera()
    ...
    def button_get_displacement(self):
        if self.timer_camera_2.isActive() == False:  # 若定时器未启动
            # 水平噪声、数值噪声
            stream = VideoGear(source=1, stabilize=self.para['is_anti_shake'], **self.solution['options']).start() # To open any valid video stream(for e.g device at 0 index)          

            self.timer_camera_2.start(50)  # 定时器开始计时30ms，结果是每过30ms从摄像头中取一帧显示
            self.button_get_displacement.setText('关闭displacement')
        else:
