# -*- coding: utf-8 -*-

# Form implementation generated from reading ui file 'd:\Desktop\EndNoteHere\researchHere\BOS\BOS-PIV\program\20210731FLOW01\Mainwindow.ui'
#
# Created by: PyQt5 UI code generator 5.15.4
#
# WARNING: Any manual changes made to this file will be lost when pyuic5 is
# run again.  Do not edit this file unless you know what you are doing.


from PyQt5 import QtCore, QtGui, QtWidgets


class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        MainWindow.setObjectName("MainWindow")
        MainWindow.resize(800, 600)
        self.centralwidget = QtWidgets.QWidget(MainWindow)
        self.centralwidget.setObjectName("centralwidget")
        self.button_open = QtWidgets.QPushButton(self.centralwidget)
        self.button_open.setGeometry(QtCore.QRect(80, 110, 75, 23))
        self.button_open.setObjectName("button_open")
        self.label_video = QtWidgets.QLabel(self.centralwidget)
        self.label_video.setGeometry(QtCore.QRect(200, 20, 341, 241))
        self.label_video.setText("")
        self.label_video.setObjectName("label_video")
        self.button_close = QtWidgets.QPushButton(self.centralwidget)
        self.button_close.setGeometry(QtCore.QRect(80, 230, 75, 23))
        self.button_close.setObjectName("button_close")
        self.button_getdisplacement = QtWidgets.QPushButton(self.centralwidget)
        self.button_getdisplacement.setGeometry(QtCore.QRect(30, 160, 171, 51))
        self.button_getdisplacement.setObjectName("button_getdisplacement")
        self.label_video_2 = QtWidgets.QLabel(self.centralwidget)
        self.label_video_2.setGeometry(QtCore.QRect(210, 290, 341, 241))
        self.label_video_2.setText("")
        self.label_video_2.setObjectName("label_video_2")
        MainWindow.setCentralWidget(self.centralwidget)
        self.menubar = QtWidgets.QMenuBar(MainWindow)
        self.menubar.setGeometry(QtCore.QRect(0, 0, 800, 22))
        self.menubar.setObjectName("menubar")
        MainWindow.setMenuBar(self.menubar)
        self.statusbar = QtWidgets.QStatusBar(MainWindow)
        self.statusbar.setObjectName("statusbar")
        MainWindow.setStatusBar(self.statusbar)

        self.retranslateUi(MainWindow)
        QtCore.QMetaObject.connectSlotsByName(MainWindow)

    def retranslateUi(self, MainWindow):
        _translate = QtCore.QCoreApplication.translate
        MainWindow.setWindowTitle(_translate("MainWindow", "MainWindow"))
        self.button_open.setText(_translate("MainWindow", "open"))
        self.button_close.setText(_translate("MainWindow", "close"))
        self.button_getdisplacement.setText(_translate("MainWindow", "get displacement"))
